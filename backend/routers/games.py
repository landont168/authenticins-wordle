...
from game_logic import check_win, compute_feedback
...
def get_game(game_id: str, db: Session = Depends(get_db)):
    ...
    return GameStateResponse(
        ...
        word=game.target_word if game.status in ("won", "lost") else None,
    )
...
def submit_guess(
    ...
    existing_count = db.query(Guess).filter(Guess.game_id == game_id).count()
    max_guesses = game.word_length + 1
